...
export class RecipeService {
    ...
    constructor(private _http: HttpClient, private _store: Store<AppState>) {
        this._authState = _store.select("authentication");
    }
    ...
    addRecipe(recipeData: Recipe): Observable<string> {
        return this._authState
            ...
            .switchMap((token: string) => this._http.post("https://recipe-list-2caaa.firebaseio.com/data.json",
                recipeData)
                .map((data) => {
    ...
    updateRecipe(id: string, recipeData: Recipe) {
        return this._authState
            .map(state => state.token)
            .switchMap((token: string) => this._http
    ...
    removeRecipe(id: string): Observable<any> {
        return this._authState
            .map(state => state.token)
            .switchMap((token: string) => this._http.delete(`https://recipe-list-2caaa.firebaseio.com/data/${id}.json`)
                .do((responceBody) => {
                    this.recipesChanged.next(this.getRecipes());
                    return responceBody;
                }
